...
// BIP39: 32 bytes entropy -> 24-Wort-Mnemonic + Checksum.
...
async function bytesToMnemonic(entropy) {
  if (entropy.byteLength !== 32) throw new Error("bytesToMnemonic expects 32 bytes");
  const bytes = new Uint8Array(entropy);
  const hash = await _sha256(bytes);
  const checksum = hash[0]; // 8 high bits

  // Build a big bit-array of 264 bits
  const bits = [];
...
async function mnemonicToBytes(mnemonic) {
  ...
  let checksum = 0;
  for (let j = 0; j < 8; j++) checksum = (checksum << 1) | bits[256 + j];
  const hash = await _sha256(entropy);
  if (hash[0] !== checksum) throw new Error("checksum mismatch (probably typo)");
  return entropy;
}

window.LehnoBip39 = { bytesToMnemonic, mnemonicToBytes, BIP39_WORDS };
